...
// Helper: Push a single event as a Lua table.
static void push_event_table(lua_State* L, SDL_Event* e) {
    if (!e) {
        lua_pop(L, 1); // Remove table if event is null (unlikely).
        return;
    }
    ...
    switch (e->type) {
        case SDL_EVENT_QUIT:
            lua_pushinteger(L, SDL_EVENT_QUIT); // Return integer SDL_EVENT_QUIT
            lua_setfield(L, -2, "type");
            break;

        case SDL_EVENT_WINDOW_CLOSE_REQUESTED:
            lua_pushinteger(L, SDL_EVENT_WINDOW_CLOSE_REQUESTED); // Return integer
            lua_setfield(L, -2, "type");
        ...
        case SDL_EVENT_KEY_UP:
            lua_pushinteger(L, e->type); // Return SDL_EVENT_KEY_DOWN or SDL_EVENT_KEY_UP
            lua_setfield(L, -2, "type");
        ...
        case SDL_EVENT_MOUSE_BUTTON_UP:
            lua_pushinteger(L, e->type); // Return SDL_EVENT_MOUSE_BUTTON_DOWN or SDL_EVENT_MOUSE_BUTTON_UP
            lua_setfield(L, -2, "type");
        ...
        case SDL_EVENT_MOUSE_MOTION:
            lua_pushinteger(L, SDL_EVENT_MOUSE_MOTION); // Return integer
            lua_setfield(L, -2, "type");
        ...
        default:
            lua_pop(L, 1); // Remove empty table for unhandled events.
            return;
    }
}
